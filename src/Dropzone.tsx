...
class Dropzone extends React.Component<IDropzoneProps, { active: boolean; dragged: (File | DataTransferItem)[] }> {
  ...
  protected nextFileIndex: number
  ...
  handleChangeStatus = (fileWithMeta: IFileWithMeta) => {
    ...
    if (meta) {
      ...
      if ((status === "ready" && this.nextFileIndex < 2 && this.nextFileIndex < this.files.length)
       || ((status === 'aborted' || status === 'error_upload' || status === "done") && this.nextFileIndex < this.files.length)) {
        this.uploadFile(this.files[this.nextFileIndex]);
        this.nextFileIndex++;
      }
      delete meta.status
  ...
  render() {
    ...
    const extra = { active, reject, dragged, accept, multiple, parallelUploads, autoProcessQueue, minSizeBytes, maxSizeBytes, maxFiles } as IExtra
